...
def generate_user_stories_node(state: AgentState) -> dict:
    """
    Generate User Stories document from transcript and analysis using template.

    Args:
        state: Current agent state

    Returns:
        Updated state with user stories document
    """
    transcript = state["transcript"]
    analysis = state["analysis"]
    session_id = state["session_id"]

    template_content = template_manager.load_template(session_id, "HISTORIAS_USUARIO")

    llm = ChatOpenAI(model="gpt-4o", temperature=0.3)

    system_prompt = f"""Eres un coach ágil experto.
Genera un documento de Historias de Usuario siguiendo EXACTAMENTE la estructura de esta plantilla:
...
"""
    ...
    paths = save_markdown_document(
        ...
        doc_type="HISTORIAS_USUARIO"
    )

    return {
        "user_stories_doc": markdown_content,
